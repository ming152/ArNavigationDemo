...
public class Group extends Mesh {
	...
	private final Vector<Mesh> mDrawChildren = new Vector<>();	//绘制集合，防止mChildren在绘制的时候数据改变

	@Override
	public void draw(GL10 gl) {
		mDrawChildren.clear();
		mDrawChildren.addAll(mChildren);
		int size = mDrawChildren.size();
		for (int i = 0; i < size; i++) {
			mDrawChildren.get(i).draw(gl);
		}
	}
